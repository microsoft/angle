...
// Uncomment the following line of code to consistently rotate the plane to be 
// more parallel to the camera plane.
    ...
    Windows::Foundation::Numerics::float4x4 const& projectionMatrix)
{
    ...
    const float bufFromClipScale  = 0.5f;
    const float bufFromClipOffset = 0.5f;
...
inline float DepthBufferPlaneFinder::GetSmoothingFramerateAdjustmentMultiplier()
{
    ...
    return std::min(1.f, static_cast<float>(timeBetweenFrames) * IDEAL_FRAME_RATE);
}
    ...
    LeastSquaresData& data) const
{
    ...
    HRESULT hr = d3dContext->Map(resolvedDepthBufferCPUMappable, 0, D3D11_MAP_READ, 0, &mappedResource);
    ...
    if (SUCCEEDED(hr) && (!m_depthBufferMappedOnGpu))
    {
    ...
    float& outDistanceToPlaneInMeters)
{
    ...
    const auto d3dContext = m_renderer->getDeviceContext();
...
void DepthBufferPlaneFinder::CreateDeviceDependentResources()
{
    ...
    auto d3dDevice = m_renderer->getDevice();

    // Create the compute shader.
    HRESULT hr = d3dDevice->CreateComputeShader(
        g_CS_DepthResolve,
        ARRAYSIZE(g_CS_DepthResolve),
        nullptr,
    ...
    if (SUCCEEDED(hr))
    {
        ...
        hr = d3dDevice->CreateBuffer(&desc, NULL, &m_constantBuffer);
    }
    ...
    if (SUCCEEDED(hr))
    {
        ...
        hr = d3dDevice->CheckFeatureSupport(
