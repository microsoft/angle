                                                 ...
                                                 const std::vector<PackedVarying> &packedVaryings)
{
    ...
    if (tempVertexShaderInfoLog.getLength() > 0)
    {
        ...
        tempVertexShaderInfoLog.getLog(static_cast<GLsizei>(tempVertexShaderInfoLog.getLength()), NULL, &tempCharBuffer[0]);
    ...
    if (usesGeometryShader())
    {
        std::string geometryHLSL =
            mDynamicHLSL->generateGeometryShaderHLSL(registers, mRenderer->isRenderingToBackBufferEnabled(), fragmentShaderD3D, packedVaryings);
...
LinkResult ProgramD3D::link(const gl::Data &data, gl::InfoLog &infoLog)
{
    ...
    if (!mDynamicHLSL->generateShaderLinkHLSL(data, mData, infoLog, registers, useViewScale, mPixelHLSL,
