                                         ...
                                         std::string *vertexHLSL) const
{
    ...
    // useViewScale isn't supported in the D3D9 renderer
    ...
    else
    {
        ...
        // If useViewScale is enabled and we're using the D3D11 renderer via Feature Level 9_*,
    ...
    if (pixelBuiltins.glFragCoord.enabled)
    {
        ...
        if (useViewScale)
        {
            ...
            // NOTE: useViewScale is only supported by D3D11 (i.e. Shader Model 4.0+).
            if (shaderModel >= 4 && mRenderer->getShaderModelSuffix() == "")
            {
                // Some assumptions:
                ...
                //    render_area_height = (2.0f / (1 - input.gl_FragCoord.y * rhw)) * gl_FragCoord.y
                //
                ...
                //    "gl_FragCoord.y = scale_factor * render_area_height - dx_ViewScale.y * gl_FragCoord.y"
                //
                // Simplifying, this becomes:
                pixelStream
                    << "    gl_FragCoord.y = (1.0f + dx_ViewScale.y) * gl_FragCoord.y /"
                       "(1 - input.gl_FragCoord.y * rhw)  - dx_ViewScale.y * gl_FragCoord.y;\n";
            }
        }
                                                    ...
                                                    const std::string &preambleString) const
{
    ...
    if (pointSprites)
    {
        ...
        for (int corner = 0; corner < 4; corner++)
        {
            ...
            {
                shaderStream << "    \n"
                                "    output.dx_Position = dx_Position + float4(1.0, "
                                "-dx_ViewScale.y, 1.0, 1.0)"
