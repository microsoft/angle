        ...
        if (argv[0][0] == '-') {
            ...
            if (compiler) {
                ...
                if (compiled && (compileOptions & SH_OBJECT_CODE)) {
                    ...
                    {
                        vertexShader = new Shader(buffer);
                        vertexShader->parseUniforms(activeUniforms);
                        vertexShader->parseVaryings();
                        vertexShader->parseAttributes();
                    }
                    else
                    {
                        fragmentShader = new Shader(buffer);
                        fragmentShader->parseUniforms(activeUniforms);
                        fragmentShader->parseVaryings();
                        fragmentShader->sortVaryings();
                    }
                }
