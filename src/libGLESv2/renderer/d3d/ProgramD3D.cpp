                                                     ...
                                                     int registers)
{
    ...
    {
        if (!mRenderer->getRendererCaps().supportsGeometryShaders)
        {
            infoLog.append("Requested shaders require Geometry Shaders, which aren't supported on current hardware.");
            return gl::LinkResult(false, gl::Error(GL_INVALID_OPERATION, "Requested shaders require Geometry Shaders, which aren't supported on current hardware."));
        }
        else
        {
