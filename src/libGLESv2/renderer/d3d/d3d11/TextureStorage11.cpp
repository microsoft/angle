                                                   ...
                                                   const gl::ImageIndex &index, const gl::Box &copyArea)
{
    ...
    gl::Error error = getResource(level, &dstTexture);
    if (error.isError())
    {
        return error;
    }
                                                 ...
                                                 const gl::ImageIndex &index, const gl::Box &region)
{
    ...
    gl::Error error = getResource(index.mipIndex, &srcTexture);
    if (error.isError())
    {
        return error;
    }
      ...
      mUseLevelZeroTexture(false)
{
    ...
    const d3d11::DXGIFormat &dxgiFormatInfo = d3d11::GetDXGIFormatInfo(mTextureFormat);
    const d3d11::TextureFormat &formatInfo = d3d11::GetTextureFormatInfo(dxgiFormatInfo.internalFormat, renderer->getFeatureLevel());
...
gl::Error TextureStorage11_2D::copyToStorage(TextureStorage *destStorage)
{
    ...
    if (mRenderer->getWorkarounds().zeroMaxLodWorkaround)
    {
        ...
        if (mTexture)
        {
            ...
            ID3D11Resource* destResource = NULL;
            gl::Error error = dest11->getResource(&destResource);
            if (error.isError())
            {
                return error;
            }
    ...
    else
    {
        ...
        ID3D11DeviceContext *immediateContext = mRenderer->getDeviceContext();
        immediateContext->CopyResource(destResource, srcResource);
    }
...
gl::Error TextureStorage11_2D::getResource(ID3D11Resource **outResource)
{
    ...
    {
        if (mLevelZeroTexture == NULL && mTextureWidth > 0 && mTextureHeight > 0)
        {
            gl::Error error = createTexture2DHelper(mTextureWidth, mTextureHeight, 1, &mLevelZeroTexture);
            if (error.isError())
            {
                return error;
            }
    ...
    else
    {
        ...
        {
            gl::Error error = createTexture2DHelper(mTextureWidth, mTextureHeight, mMipLevels, &mTexture);
            if (error.isError())
            {
                return error;
            }
...
gl::Error TextureStorage11_2D::getResource(int includeLevel, ID3D11Resource **outResource)
{
...
gl::Error TextureStorage11_2D::createTexture2DHelper(GLsizei width, GLsizei height, int levels, ID3D11Texture2D** texture)
{
    ASSERT(mMipLevels > 0);
...
gl::Error TextureStorage11_2D::getRenderTarget(const gl::ImageIndex &index, RenderTarget **outRT)
{
    ...
    if (!mRenderTarget[level])
    {
        ...
        if (mRenderTargetFormat != DXGI_FORMAT_UNKNOWN)
        {
            ...
            mRenderTarget[level] = new RenderTarget11(mRenderer, rtv, texture, srv, getLevelWidth(level), getLevelHeight(level), 1, false);
        ...
        else if (mDepthStencilFormat != DXGI_FORMAT_UNKNOWN)
        {
            ...
            mRenderTarget[level] = new RenderTarget11(mRenderer, dsv, texture, srv, getLevelWidth(level), getLevelHeight(level), 1, false);
...
gl::Error TextureStorage11_Cube::getRenderTarget(const gl::ImageIndex &index, RenderTarget **outRT)
{
    ...
    if (!mRenderTarget[faceIndex][level])
    {
        ...
        if (mRenderTargetFormat != DXGI_FORMAT_UNKNOWN)
        {
            ...
            mRenderTarget[faceIndex][level] = new RenderTarget11(mRenderer, rtv, texture, srv, getLevelWidth(level), getLevelHeight(level), 1, false);
        ...
        else if (mDepthStencilFormat != DXGI_FORMAT_UNKNOWN)
        {
            ...
            mRenderTarget[faceIndex][level] = new RenderTarget11(mRenderer, dsv, texture, srv, getLevelWidth(level), getLevelHeight(level), 1, false);
...
gl::Error TextureStorage11_3D::getRenderTarget(const gl::ImageIndex &index, RenderTarget **outRT)
{
    ...
    if (!index.hasLayer())
    {
        if (!mLevelRenderTargets[mipLevel])
        {
            ...
            mLevelRenderTargets[mipLevel] = new RenderTarget11(mRenderer, rtv, texture, srv, getLevelWidth(mipLevel), getLevelHeight(mipLevel), getLevelDepth(mipLevel), false);
    ...
    else
    {
        ...
        if (mLevelLayerRenderTargets.find(key) == mLevelLayerRenderTargets.end())
        {
            ...
            mLevelLayerRenderTargets[key] = new RenderTarget11(mRenderer, rtv, texture, srv, getLevelWidth(mipLevel), getLevelHeight(mipLevel), 1, false);
...
gl::Error TextureStorage11_2DArray::getRenderTarget(const gl::ImageIndex &index, RenderTarget **outRT)
{
    ...
    if (mRenderTargets.find(key) == mRenderTargets.end())
    {
        ...
        if (mRenderTargetFormat != DXGI_FORMAT_UNKNOWN)
        {
            ...
            mRenderTargets[key] = new RenderTarget11(mRenderer, rtv, texture, srv, getLevelWidth(mipLevel), getLevelHeight(mipLevel), 1, false);
