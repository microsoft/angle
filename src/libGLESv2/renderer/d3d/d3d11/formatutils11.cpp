...
static DXGIFormatInfoMap BuildDXGIFormatInfoMap()
{
    ...
    AddDXGIFormat(&map, DXGI_FORMAT_B5G6R5_UNORM,             16,  1, 1, GL_UNSIGNED_NORMALIZED, NULL,                       NULL);
    AddDXGIFormat(&map, DXGI_FORMAT_B5G5R5A1_UNORM,           16,  1, 1, GL_UNSIGNED_NORMALIZED, NULL,                       NULL);
    AddDXGIFormat(&map, DXGI_FORMAT_B4G4R4A4_UNORM,           16,  1, 1, GL_UNSIGNED_NORMALIZED, NULL,                       NULL);
...
static D3D11ES3FormatMap BuildD3D11_FL9_3FormatOverrideMap()
{
    ...
    // Depth stencil formats   | GL internal format    | D3D11 texture format          | D3D11 SRV format      | D3D11 RTV format      | D3D11 DSV format
...
const TextureFormat &GetTextureFormatInfo(GLenum internalFormat, D3D_FEATURE_LEVEL d3dFeatureLevel)
{
    ...
    {
        // First see if the internalFormat has a special mapping for FL9_3
...
static D3D11VertexFormatInfoMap BuildD3D11_FL9_3VertexFormatInfoOverrideMap()
{
    ...
    // GL_HALF_FLOAT
    // TODO: Add test to verify that these work correctly.
    AddVertexFormatInfo(&map, GL_HALF_FLOAT,     GL_FALSE,  1, VERTEX_CONVERT_CPU,      DXGI_FORMAT_R16G16_FLOAT,        &CopyNativeVertexData<GLhalf, 1, 2, 0>);
    // NOTE: 2, 3 and 4 component GL_HALF_FLOAT should use the default format table.
    ...
    // TODO: Add test to verify that this works correctly.
    AddVertexFormatInfo(&map, GL_UNSIGNED_INT_2_10_10_10_REV, GL_TRUE, 4, VERTEX_CONVERT_CPU, DXGI_FORMAT_R32G32B32A32_FLOAT, &CopyXYZ10W2ToXYZW32FVertexData<false, true,  true>);
...
const VertexFormat &GetVertexFormatInfo(const gl::VertexFormat &vertexFormat, D3D_FEATURE_LEVEL d3dFeatureLevel)
{
